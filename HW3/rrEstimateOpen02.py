	...
	for i in range(actionCount):
		...
		if day >= preDay and day >= 0 and z > 0 :
			...
			if a == -1 and b >= 0 and capital > 0 :	 # Suggested action is "buy"
				...
				realAction[i] = 1
				# print(f"days: {day} a: {a}, b: {b}, stockHolding[i][a]: {stockHolding[i][a] if a >= 0 else 'N/A'}, capital: {capital}")
				...
				print(f"Debug info - days: {day} a: {a}, b: {b}, stockHolding[i][a]: {stockHolding[i][a] if a >= 0 else 'N/A'}, capital: {capital}")
				assert False
		else:
			print(f"Debug info - day: {day}, preDay: {preDay}, z: {z}")
			assert False
